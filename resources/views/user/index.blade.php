<x-app-layout>
            ...
            <div class="lg:flex justify-around">
                <di class="lg:flex items-center">
                    <select name="category" class="mb-2 lg:mb-0 lg:mr-2">
                        ...
                        @foreach($categories as $category)
                            ...
                            @foreach($category->secondary as $secondary)
                                <option value="{{ $secondary->id}}" @if(\Request::get('category') == $secondary->id) selected @endif>{{--selectタグの中ではoptionタグで表示する必要がある--}}
                                    {{ $secondary->name }}
                                </option>
                            @endforeach
                        @endforeach
                    ...
                    <div class="flex space-x-2 items-center">
                        <div><input name="keyword" class="border border-gray-500 py-2" placeholder="キーワードを選択"></div>
  ...
  <script>
    const select = document.getElementById('sort');//これは上記でidを指定した'sort'が入ってくる
    select.addEventListener('change',function(){
        this.form.submit();
    })

    const paginate = document.getElementById('pagination');//これは上記でidを指定した'sort'が入ってくる
